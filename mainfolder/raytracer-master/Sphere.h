...
{	
	...
	virtual double findIntersection(Ray ray)
	{
		...
		double c = pow(ray_origin_x - sphere_center_x, 2) + pow(ray_origin_y - sphere_center_y, 2) + pow(ray_origin_z - sphere_center_z, 2) - pow(radius, 2);
		...
		if (discriminant > 0)
		{
			...
			//first root for intersection
			double root_1 = ((-1*b - sqrt(discriminant))/2) - 0.000001;
			
			if (root_1 > 0)
			{
				// first root is nearest to camera
				return root_1;
			}
			else 
			{
				//second root is nearest to camera
